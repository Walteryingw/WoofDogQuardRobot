...
SoftwareSerial BTSerial(10, 11);   //蓝牙模块引脚接10，11
...
void servopwm()//关节计算角度对应到舵机转角, pwm.writeMicroseconds用的是毫秒,还有pwm.setPWM(),用的是脉冲
{
  ...
  pwm.writeMicroseconds(LBS, d2p(J3 + s3)); //腿3肩关节横向转角    // SPOTMICRO肩关节，WOOFDOG 改成-
  pwm.writeMicroseconds(RBS, d2p(J4 - s4)); //腿4肩关节横向转角    // SPOTMICRO肩关节，WOOFDOG 改成+
  pwm.writeMicroseconds(LFS, d2p(J1 - s1)); //腿1肩关节横向转角    // SPOTMICRO肩关节，WOOFDOG 改成+
  pwm.writeMicroseconds(RFS, d2p(J2 + s2)); //腿2肩关节横向转角    // SPOTMICRO肩关节，WOOFDOG 改成-
...
void rests() { //休息
  pwm.writeMicroseconds(LBS, d2p(J3));//腿3肩关节横向转角
  pwm.writeMicroseconds(LR_H, d2p(D3 + 25)); //腿3大腿转角
  pwm.writeMicroseconds(LR_S, d2p(X3 - 35)); //腿3小腿转角

  pwm.writeMicroseconds(RBS, d2p(J4));//腿4肩关节横向转角
  pwm.writeMicroseconds(RR_H, d2p(D4 - 25)); //腿4大腿转角
  pwm.writeMicroseconds(RR_S, d2p(X4 + 35)); //腿4小腿转角

  pwm.writeMicroseconds(LFS, d2p(J1));//腿1肩关节横向转角
  pwm.writeMicroseconds(LF_H, d2p(D1 + 25)); //腿1大腿转角
  pwm.writeMicroseconds(LF_S, d2p(X1 - 35)); //腿1小腿转角

  pwm.writeMicroseconds(RFS, d2p(J2));//腿2肩关节横向转角
  pwm.writeMicroseconds(RF_H, d2p(D2 - 25)); //腿2大腿转角
  pwm.writeMicroseconds(RF_S, d2p(X2 + 35)); //腿2小腿转角
...
void mpu6050()//自稳模块
{
  ...
  */
  ...
  D = -mpu.getAngleX() / 180 * PI; //修正  /////////////  此处根据MPU实际安装的方向需做调整
  A = -mpu.getAngleY() / 180 * PI; //修正   /////////////
  /*  D = -mpu.getAngleX() / 180 * PI; //修正  /////////////  mpu6050_token
    A = -mpu.getAngleY() / 180 * PI; //修正   /////////////
  */
}
...
void wentai()//静态动作方程
{
  ...
  x3 = Ht3 * cos(A) * sin(A);
  //x3 = Ht3 * cos(A) * sin(A) + (1 / 2 * cos(0.2 * PI + Z)-w);
...
void balance()//自稳函数
{
  ...
  c =  - (w / 2) * sin(D); //90为四足机器人矩形宽度的一半
  b = Ht - (l / 2) * sin(A); //120为四足机器人矩形长度的一半
  Ht3 = c + b - Ht; //腿3高度
  Ht2 = 2 * Ht - Ht3; //腿2高度
  Ht1 = 2 * c - Ht3; //腿1高度
  Ht4 = 2 * Ht - Ht1; //腿4高度
...
{ A = -AX / 180 * PI;
  ...
  c = Ht - (w / 2) * sin(D); //90为四足机器人矩形宽度的一半
  b = Ht - (l / 2) * sin(A); //120为四足机器人矩形长度的一半
  Ht3 = c + b - Ht; //腿3高度
  Ht2 = 2 * Ht - Ht3; //腿2高度
  Ht1 = 2 * c - Ht3; //腿1高度
